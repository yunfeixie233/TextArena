...
class DiplomacyState(ta.State):
    """
    Extended State class that provides backward compatibility with old TextArena API
    while working with the new core.
    """
    
    def __init__(self, num_players: int, seed: Optional[int] = None, max_turns: Optional[int] = None,
                 min_players: Optional[int] = None, max_players: Optional[int] = None,
                 role_mapping: Optional[Dict[int, str]] = None):
        # Validate player count if min/max specified
        if min_players is not None and num_players < min_players:
            raise ValueError(f"The number of players needs to be at least {min_players}. You provided {num_players}")
        if max_players is not None and num_players > max_players:
            raise ValueError(f"The number of players needs to be at most {max_players}. You provided {num_players}")
        ...
        self._role_mapping[ta.GAME_ID] = "GAME"
        
        # Track state for backward compatibility
        self.done = False
        self.info = {}
        self.rewards = None
    ...
    def reset(self, game_state: Dict[str, Any], player_prompt_function, 
              executable_on_reset: Optional[List] = None, seed: Optional[int] = None,
              role_mapping: Optional[Dict[int, str]] = None):
        """Backward compatibility reset method"""
        if role_mapping:
            self._role_mapping.update(role_mapping)
            
        # Use the new standard_resets method
        self.standard_resets(
            game_state=game_state,
            player_prompt_function=player_prompt_function,
            role_mapping=self._role_mapping
        )
        
        # Execute any additional reset functions
        if executable_on_reset:
            for executable in executable_on_reset:
                executable()

    def add_observation(self, message: str, observation_type: ObservationType, 
                        from_id: int = ta.GAME_ID, to_id: int = -1):
        """
        New-style add_observation method matching the core signature
        """
        # Call the parent method directly
        super().add_observation(
            message=message,
    ...
    def add_observation_old_style(self, from_id: int, to_id: int, message: str, for_logging: bool = True):
        """
        Backward compatibility method for old-style add_observation calls
        """
        # Determine observation type based on from_id
        if from_id == ta.GAME_ID:
    ...
    def step(self, rotate_player: bool = True):
        ...
        # Handle player rotation manually if needed
    ...
    def manually_update_current_player(self, new_player_id: int):
        """Backward compatibility method for manual player updates"""
    ...
    def set_winners(self, player_ids: List[int], reason: Optional[str]):
        ...
        # Add observation and set done state
        if len(player_ids) == 1:
            message = f"Player {player_ids[0]} won the game. Reason: {reason}"
        else:
            message = f"Players {player_ids} won the game. Reason: {reason}"
            
        self.add_observation_old_style(
            from_id=ta.GAME_ID,
            to_id=-1,
            message=message,
            for_logging=False
    ...
    def close(self):
        """Close the game and return rewards and game_info"""
...
class DiplomacyEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        ...
        self.state = DiplomacyState(
            num_players=num_players, 
            min_players=3, 
            max_players=7, 
            seed=seed,
            max_turns=self.max_turns  # Use game engine's turn limit for consistency
        )
        
        # Initialize game engine
        self.engine = DiplomacyGameEngine(max_turns=self.max_turns)
        self.player_power_map = self.engine.setup_game(num_players)
        self.state._role_mapping = self.player_power_map
        ...
        # Initialize the state using the backward compatibility method
        self.state.reset(
            game_state=game_state,
            player_prompt_function=player_prompt_function,
        )
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        """
        # FIRST: Check if game is already completely over
        if self.state.done:
            done, info = self.state.step(rotate_player=False)
            return done, info
            
        # SECOND: Check if game engine detected victory but state not updated yet
        if self.engine.game_over:
            self._announce_game_result()
            # Force done state to ensure termination
    ...
    def _announce_game_result(self):
        ...
        for entry in self.engine.order_history:
            announcement += f"- {entry['season']} {entry['year']} {entry['phase']}: {len(entry.get('valid_orders', {}))} powers submitted orders\n"
    ...
    def add_observation(self, from_id: int, to_id: int, message: str):
        ...
        self.state.add_observation_old_style(from_id=from_id, to_id=to_id, message=message)
    ...
    def get_board_str(self) -> str:
        """Get the ASCII representation of the current game board"""
        return self.engine.get_ascii_map()
    
    def close(self):
        """Close the game and return both rewards and game_info"""
