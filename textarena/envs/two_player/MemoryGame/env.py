    ...
    def step(
        ...
        if match is None:
            ...
            if r1 < 0 or r1 >= self.grid_size or c1 < 0 or c1 >= self.grid_size or r2 < 0 or r2 >= self.grid_size or c2 < 0 or c2 >= self.grid_size:
                self.state.set_invalid_move(
                    player_ids=[player_id],
                    reasons=[f"Invalid move. Player {player_id} selected an out-of-bounds position."]
                )
            elif (r1, c1) == (r2, c2):
                self.state.set_invalid_move(
                    player_ids=[player_id],
                    reasons=[f"Invalid move. Player {player_id} selected the same card twice."]
                )
            elif (r1, c1) in self.matched_positions or (r2, c2) in self.matched_positions:
                self.state.set_invalid_move(
                    player_ids=[player_id],
                    reasons=[f"Invalid move. Player {player_id} selected one or both cards that have already been matched."]
                )
            else:
