...
class LetterAuctionEnv(ta.Env):
    ...
    def __init__(
        ...
        self.state = ta.State(
            num_players=2,
            render_keys=["rendered_text", "turn"]
        )
        ...
        if self.round_number < len(self.letters):
            ...
            if not match:
                ...
                self.state.set_invalid_move(
                    ...
                    reasons=[f"Invalid action: {action}. Please enter a valid action: '[bid <amount>]' or '[pass]'."]
                )
            ...
            if not match:
                ...
                self.state.set_invalid_move(
                    player_ids=[player_id],
                    reasons=[f"Invalid action: {action}. Please enter a valid action: '[<word>]'."]
                )
    ...
    def _place_bid(
        ...
        if self.player_states[player_id]["coins"] < bid_amount:
            ...
            self.state.set_invalid_move(
                player_ids=[player_id],
                reasons=[f"Invalid bid: {bid_amount}. You do not have enough coins."]
            )
            return
    ...
    def _calculate_word_value(
        ...
        if en_us_dict.check(word) == False and en_uk_dict.check(word) == False:
            ...
            self.state.set_invalid_move(
                player_ids=[player_id],
                reasons=[f"Invalid word: {word}. Please enter a valid English word."]
            )
            return
        ...
        for letter in word:
            if letter not in self.player_states[player_id]["letters"]:
                ...
                self.state.set_invalid_move(
                    player_ids=[player_id],
                    reasons=[f"Invalid word: {word}. You do not have the letter '{letter}'."]
                )
                return
    ...
    def _generate_player_prompt(self, player_id) -> str:
        """
