...
class LiarsDiceEnv(ta.Env):
    ...
    def __init__(self, num_players: int = 3, num_dice: int = 5):
        """
        ...
        self.state = ta.State(num_players=num_players, max_turns=None)
    ...
    def terminal_render_keys(self):
        return [
            *[["dice_rolls", pid] for pid in range(self.num_players)], 
            "remaining_dice", 
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        remaining_dice = {pid: self.initial_num_dice for pid in range(self.num_players)}
        dice_rolls = {
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        Args:
            action (str): The action string (either a '[Bid: Q, F]' or '[Call]').

        Returns:
            done (bool): Whether the game is finished.
            info (ta.Info): Additional info about the step.
        """
        ...
        if self.call_pattern.search(action):
            ...
            if last_bidder_id is None or current_bid["quantity"] == 0:
                ...
                self.state.set_invalid_move(
                    player_id=player_id,
                    reason="Call made with no prior bid."
                )
    ...
    def _rotate_players(self):
        ...
        while next_player_id != current_player_id:
            ...
            raise Exception(f"Tried rotating with no players left")
