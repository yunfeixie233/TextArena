...
class ColonelBlottoEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        ...
        self.state.reset(game_state=initial_game_state, player_prompt_function=self._prompt, role_mapping={0: "Commander Alpha", 1: "Commander Beta"})
    ...
    def _render_game_state(self) -> str:
        ...
        lines = []
        lines.append(f"=== COLONEL BLOTTO - Round {current_round}/{self.num_rounds} ===")
        ...
        if phase == 'allocation':
            ...
            if player_state['allocation_complete']:
                ...
                return (
                    ...
                    f"Format: A:4, B:2, C:2 (must sum to exactly {self.num_total_units})\n"
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        self.state.add_observation(message=f"Current game state:\n{self._render_game_state()}", observation_type=ta.ObservationType.GAME_BOARD)
    ...
    def _validate_allocation(self, allocation_dict: Optional[Dict[str, int]]) -> str:
        ...
        if len(allocation_dict)!=len(self.field_names):                                         return f"Must allocate to all {len(self.field_names)} fields." # Check if all required fields are present first
        if any(field not in self.field_names for field in allocation_dict.keys()):              return f"Invalid field name(s). Valid fields: {', '.join(self.field_names)}" # Check for invalid field names
        if any(not isinstance(units, int) or units < 0 for units in allocation_dict.values()):  return "All allocations must be non-negative integers." # Check for non-negative integers
        if sum(allocation_dict.values()) != self.num_total_units:                               return f"Units must sum to exactly {self.num_total_units}. Current sum: {sum(allocation_dict.values())}" # Check sum last (after we know all fields are present and valid)
    ...
    def _resolve_battle(self):
        ...
        if round_winner is not None:
            ...
            self.state.add_observation(message=f"Round {self.state.game_state['current_round']} winner: {winner_name}!", observation_type=ta.ObservationType.GAME_ACTION_DESCRIPTION)
        else:
            self.state.add_observation(message=f"Round {self.state.game_state['current_round']} ended in a tie!", observation_type=ta.ObservationType.GAME_ACTION_DESCRIPTION)
    ...
    def _transition_to_next_round(self):
        ...
        for player_id in [0, 1]:
            ...
            self.state.game_state['player_states'][player_id]['current_allocation'] = {field_name: 0 for field_name in self.field_names}
    ...
    def _check_gameover(self):
        ...
        if self.state.check_turn_limit():
            if scores[0] > scores[1]:   self.state.set_winner(player_id=0, reason=f"Turn limit reached. Commander Alpha wins {scores[0]}-{scores[1]}!")
            elif scores[1] > scores[0]: self.state.set_winner(player_id=1, reason=f"Turn limit reached. Commander Beta wins {scores[1]}-{scores[0]}!")
            else:                       self.state.set_draw(reason=f"Turn limit reached. Game ends in a {scores[0]}-{scores[1]} tie!")