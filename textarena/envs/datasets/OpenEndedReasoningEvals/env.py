...
class OpenEndedReasoningEvalsEnv(ta.Env):
    ...
    def _show_next_question(self):
        ...
        try:
            ...
            self.state.add_observation(from_id=ta.GAME_ID, to_id=self.state.current_player_id, message=str(self.question)) # Add question to state
            # return True
    ...
    def _check_answer(self, submitted_answer: str) -> bool:
        # TODO 
        # to standardize, should extract using [...], and rather introduce a wrapper that extracts it from //boxed{} -> [...]



#         """ Check if the submitted answer contains the correct answer """
#         boxed_pattern = r"\\boxed\{(\d+)\}" # Look for boxed answers in LaTeX
#         boxed_matches = re.findall(boxed_pattern, submitted_answer)
#         if boxed_matches: # If we found boxed numbers, check those
#             for match in boxed_matches:
#                 if match.strip() == self.answer.strip():
#                     return True
#         number_pattern = r"\b" + re.escape(self.answer) + r"\b" # Check for the number anywhere in the text
#         if re.search(number_pattern, submitted_answer):
#             return True
#         try: # Fall back to the regular math_eval
#             return math_eval(submitted_answer, self.answer)
#         except Exception as e:
#             return submitted_answer.strip() == self.answer.strip() # Basic exact match as last resort
