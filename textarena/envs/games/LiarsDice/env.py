...
class LiarsDiceEnv(ta.Env):
    ...
    def reset(self, num_players:int, seed: Optional[int] = None):
        ...
        dice_rolls = {pid: [random.randint(1, 6) for _ in range(self.initial_num_dice)] for pid in range(self.state.num_players)}
    ...
    def _roll_new_dice(self):
        """ Roll new dice for each player *who still has dice* after a round ends, and reset the current bid """
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if self.call_pattern.search(action):
            ...
            if total_face_count < quantity:
                ...
                msg = f"Player {player_id} calls! The actual count of face {face_value} is {total_face_count}, which is LESS than {quantity}.\nPlayer {loser_id} (the last bidder) loses one die."
            else:
                # Otherwise, the caller loses a die
                loser_id = player_id
                msg = f"Player {player_id} calls! The actual count of face {face_value} is {total_face_count}, which is >= {quantity}.\nPlayer {loser_id} (the caller) loses one die."
        ...
        if bid_match:
            ...
            if is_valid:
                ...
                self.state.add_observation(message=f"Player {player_id} bids {new_quantity} of face {new_face_value}.")
            else:
        ...
        self.state.set_invalid_move(player_id=player_id, reason="Action not recognized as either a valid '[Bid: X, Y]' or '[Call]'.")
        return self.state.step(rotate_player=False)
    ...
    def _apply_die_loss(self, loser_id: int, message: str):
        ...
        if self.state.game_state["remaining_dice"][loser_id] == 0:
            ...
            players_remaining = [pid for pid, count in self.state.game_state["remaining_dice"].items() if count > 0]
    ...
    def _is_valid_bid(self, new_quantity: int, new_face_value: int, current_bid: Dict[str, int]) -> Tuple[bool, str]:
        ...
        if new_quantity < old_quantity: return False, f"New quantity {new_quantity} is lower than current {old_quantity}."
        if new_face_value < old_face_value: return False, f"New face value {new_face_value} is lower than current {old_face_value}."
        if new_quantity == old_quantity and new_face_value == old_face_value: return False, "Bid is identical to the current bid."
