...
class MinesweeperEnv(ta.Env):
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if match is None:
            self.state.set_invalid_move(reason="You did not respond with valid coordinates in square brackets.")
        else:
            ...
            if not (0 <= row < self.rows and 0 <= col < self.cols):
                ...
                if self.revealed[row][col]:
                    self.state.set_invalid_move(reason=f"The cell at ({row}, {col}) has already been revealed.")
                else:
