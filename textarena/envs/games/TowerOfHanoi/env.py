...
class TowerOfHanoiEnv(ta.Env):
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if not matches:
            self.state.set_outcome(reward=self._get_percentage_completion(), reason="You did not respond with valid '[source] [target]'.")
        else:
            for match in matches:
                ...
                if source not in self.state.game_state['towers'] or target not in self.state.game_state['towers']: 
                    self.state.set_outcome(reward=self._get_percentage_completion(), reason="You specified an invalid source or target tower."); break
                elif not self.state.game_state['towers'][source]:
                    self.state.set_outcome(reward=self._get_percentage_completion(), reason="You tried to move a disk from an empty tower."); break
                elif self.state.game_state['towers'][target] and self.state.game_state['towers'][target][-1] < self.state.game_state['towers'][source][-1]:
                    self.state.set_outcome(reward=self._get_percentage_completion(), reason="You tried to place a larger disk on a smaller disk.")
                else:
