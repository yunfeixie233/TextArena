...
from textarena.envs.games.ScenarioPlanning.renderer import create_board_str
    ...
    def _load_scenarios(self, scenarios_path: Optional[str]):
        try:
            if scenarios_path is not None: # Use provided path
                ...
                with importlib.resources.files('textarena.envs.games.ScenarioPlanning').joinpath('scenarios.json').open('r') as file: self.scenarios = json.load(file)["scenarios"]
