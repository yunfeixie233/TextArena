...
class PokerEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        ...
        self.state = ta.FFAMultiPlayerState(num_players=num_players, seed=seed) # Create the underlying state for N players
    ...
    def _observe_current_pot(self):
        ...
        self.state.add_observation(to_id=current_pid, message=obs_message, observation_type=ta.ObservationType.GAME_MESSAGE)
    ...
    #     current_pid = self.state.current_player_id
    #     self.state.add_observation(from_id=current_pid, message=action, observation_type=ta.ObservationType.PLAYER_ACTION) # Broadcast the action
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        if self.state.game_state["game_complete"]:
            ...
            self.state.set_invalid_move(player_id=self.state.current_player_id,
                                        reason=reason)
            return False, {"reason": reason}

        cur = self.state.current_player_id
        self.state.add_observation(from_id=cur,
                                   message=action,
                                   observation_type=ta.ObservationType.PLAYER_ACTION)

        self._process_betting_action(action=action, player_id=cur)

        # If the betting round ended inside _process_betting_action
        # (e.g. everyone checked / all-in), _handle_hand_completion()
        # will already have been called – in that case _rotate_players
        # will realise the hand is done and do nothing further.
        self._rotate_players()

        # Show the current player the updated table view
        self._observe_current_pot()
        return self.state.step(rotate_player=False)
    
    def _rotate_players(self):
        """
        Move the turn to the next *alive* player, skipping busts,
        folded players for the rest of the current hand, and anyone
        that is already all-in.  If no such player exists we finish
        the hand (→ showdown) or, if the tournament is done,
        we call _set_outcome().
        """
        next_pid = self.state.next_alive_player(
            predicate=lambda pid: (
                pid not in self.state.game_state["folded_players"] and
                pid not in self.state.game_state["all_in_players"]
            )
        )

        # next_alive_player() returns None when only one (or zero) live
        # players satisfy the predicate.
        if next_pid is None:
            self._handle_hand_completion()        # finish this pot
        else:
    ...
    def _apply_action(self, player_id: int, action_type: str, bet_amount: Optional[int]):
        ...
        if gs["player_chips"][player_id] == 0 and player_id not in gs["all_in_players"]:
            ...
            self.state.add_observation(message=f"Player {player_id} has 0 chips and is automatically considered all-in.", observation_type=ta.ObservationType.GAME_MESSAGE)
        
        if action_type == "fold":
            gs["folded_players"].add(player_id)
            self.state.add_observation(message=f"Player {player_id} folds.", observation_type=ta.ObservationType.GAME_MESSAGE)
            ...
            if gs["current_bet"] > gs["player_bets"][player_id]:
                ...
                self.state.add_observation(message=f"Player {player_id} checks.", observation_type=ta.ObservationType.GAME_MESSAGE)
            ...
            if call_needed < 0:
                ...
                gs["checked_players"].add(player_id)
                self.state.add_observation(message=f"Player {player_id} checks (already matched).", observation_type=ta.ObservationType.GAME_MESSAGE)
            ...
            if call_needed >= gs["player_chips"][player_id]:
                ...
                gs["all_in_players"].add(player_id)
                self.state.add_observation(message=f"Player {player_id} calls all-in for {all_in_amount}.", observation_type=ta.ObservationType.GAME_MESSAGE)
                ...
                gs["pot"] += call_needed
                self.state.add_observation(message=f"Player {player_id} calls {call_needed}.", observation_type=ta.ObservationType.GAME_MESSAGE)
            ...
            self.state.add_observation(message=f"Player {player_id} {action_type}s {needed} (total={gs['player_bets'][player_id]}).", observation_type=ta.ObservationType.GAME_MESSAGE)
    ...
    def _handle_showdown(self):
        ...
        if len(active_players) == 1:
            ...
            self.state.add_observation(message=f"Player {winner} wins the pot by default (everyone else folded).", observation_type=ta.ObservationType.GAME_MESSAGE)
            gs["pot"] = 0
            return
        ...
        self.state.add_observation(message="Showdown:\n" + "\n".join(details), observation_type=ta.ObservationType.GAME_MESSAGE)
        ...
        # Award the pot
        if len(winners) == 1:
            gs["player_chips"][winners[0]] += gs["pot"]
            self.state.add_observation(message=f"Player {winners[0]} wins the pot of {gs['pot']}.", observation_type=ta.ObservationType.GAME_MESSAGE)
            gs["pot"] = 0
        else:
            ...
            gs["pot"] = 0
            self.state.add_observation(message=f"Tie among players {winners}. Each gets {split_amt} chips.", observation_type=ta.ObservationType.GAME_MESSAGE)
            if remainder > 0:
    ...
    def _advance_game_phase(self):
        ...
        if gs["betting_round"] < 3:
            ...
            self.state.add_observation(message=reveal_message, observation_type=ta.ObservationType.GAME_MESSAGE)
    ...
    def determine_winner(self) -> Tuple[List[int], int]:
        ...
        if len(winners) == 1:
            ...
            self.state.add_observation(message=f"Game complete! {standings_msg}", observation_type=ta.ObservationType.GAME_MESSAGE)
        else:
            # tie
    ...
    def _check_and_eliminate(self, pid: int):
        "Send a player to the rail as soon as their stack hits 0."
        if self.state.game_state["player_chips"][pid] == 0:
            self.state.add_elimination(pid=pid)

    def _set_outcome(self):
        """
        End-of-game pay-out identical to LiarsDice: linear scale
        –1 … +1 according to final ranking.  Ranking is decided by
        elimination order first, then by chip count (higher is better)
        for the players that survived to the end-game showdown.
        """
        gs = self.state.game_state
        # 1) players that already busted – we have that list
        ranking = list(self.state.elimination_order)
        # 2) anyone still alive – order them by stack, small → large
        survivors = [
            pid for pid in range(self.state.num_players)
            if pid not in ranking
        ]
        survivors.sort(key=lambda pid: gs["player_chips"][pid])
        ranking.extend(survivors)          # now lowest-chips first

        # linear reward in [-1, +1]
        rewards = {
            pid: -1.0 + 2.0 * rank / (self.state.num_players - 1)
            for rank, pid in enumerate(ranking)
        }
        self.state.set_game_outcome(
            reward_dict=rewards,
            reason=f"Player {ranking[-1]} wins! Final standing: {ranking}",
        )