...
class MastermindEnv(ta.Env):
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if match is None:
            self.state.set_invalid_move(reason=f"You did not respond with a space-separated list of numbers wrapped in square brackets.")
        ...
        try:
            ...
            self.state.set_invalid_move(reason=f"All entries must be valid integers.")
            return self.state.step()

        # Validate guess length
        if len(player_guess) != self.state.game_state["code_length"]:
            self.state.set_invalid_move(reason=f"The guess should contain exactly {self.state.game_state['code_length']} numbers.")
            return self.state.step()

        # Validate number range
        if any(num < 1 or num > self.state.game_state["num_numbers"] for num in player_guess):
            self.state.set_invalid_move(reason=f"All numbers must be between 1 and {self.state.game_state['num_numbers']}.")
            return self.state.step()
        ...
        if not self.state.game_state["duplicate_numbers"] and len(set(player_guess)) != len(player_guess):
            self.state.set_invalid_move(reason=f"Duplicate numbers are not allowed.")
            return self.state.step()
        ...
        if black_pegs == self.state.game_state["code_length"]: # Check for win condition
            ...
            self.state.add_observation(from_id=ta.GAME_ID, message=f"Submitted [{match.group(1)}]. Feedback: {black_pegs} black peg(s), {white_pegs} white peg(s).")

        # check turn count
        if self.state.check_turn_limit():
            pct_completion = black_pegs / self.state.game_state['code_length']
