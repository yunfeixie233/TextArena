...
class FFAMultiPlayerState(ta.State):
    ...
    def next_alive_player(self) -> Optional[int]:
        # return the next alive pid
        next_pid = (self.current_player_id+1) % self.num_players
        while next_pid != self.current_player_id:
            if next_pid not in self.elimination_order: return next_pid # check if alive
            next_pid = (next_pid+1) % self.num_players
        return None
